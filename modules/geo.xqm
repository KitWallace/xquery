(:~ 
    ...
    :  @since December 5, 2010 - added Grid to OS 
...
declare function geo:string-pad($char as xs:string,$n as xs:integer) {
    string-join(for $i in 1 to $n return $char,"")
};
declare function geo:lz ($n as xs:string?) as xs:integer {
    xs:integer(concat (geo:string-pad("0",2 - string-length($n)),$n))
};

declare function geo:round($n as xs:integer,$decimals as xs:integer) {
...
declare function geo:LatLong-as-kml($p as element(geo:LatLong)) as xs:string {
...
declare function geo:OS-to-Grid($easting  as xs:string, $northing as xs:string, $n as xs:integer) {
     ...
     let $prefix := $geo:grid/square[@x=$ep][@y=$np]/@prefix
  (:   let $e := error((),"x",$es)  :)
...
declare function geo:Grid-to-OS($OSG as xs:string)  as element(geo:Mercator) {
   ...
   let $square := $geo:grid/square[@prefix=$prefix]
   let $precision := string-length($ne) div 2
   let $easting := xs:integer(concat($square/@x,substring($ne,1,$precision),geo:string-pad("0",5 - $precision)))
   let $northing := xs:integer(concat($square/@y,substring($ne,$precision+1),geo:string-pad("0",5 - $precision)))
   return 
...
declare function geo:WGS84-to-Airy1830 () {
  <geo:HelmertTransformation 
        x="-446.448" y="125.157" z="-542.060" 
        rx="{geo:seconds-as-radians(-0.1502)}" ry="{geo:seconds-as-radians(-0.2470)}" rz="{geo:seconds-as-radians(-0.8421) }"  
        s="{1  - 20.4894 * 0.000001}"
...
declare function geo:LatLong-to-OS($ll as element(geo:LatLong)) as element(geo:Mercator) {
    let $e1 := $geo:Airy1830
    let $e2 := $geo:WGS84
    
    let $xyz := geo:LatLong-to-XYZ($ll, $e2)
    let $newXYZ:= geo:transform-XYZ($xyz, geo:WGS84-to-Airy1830()) 
    let $os :=  geo:XYZ-to-LatLong($newXYZ,$e1)     
    return  geo:LatLong-to-Mercator($os,$e1, $geo:UKOS)
...
<Grid>
    <square prefix="SV" y="0" x="0"/>
    <square prefix="SW" y="0" x="1"/>
    <square prefix="SX" y="0" x="2"/>
    <square prefix="SY" y="0" x="3"/>
    <square prefix="SZ" y="0" x="4"/>
    <square prefix="TV" y="0" x="5"/>
    <square prefix="SR" y="1" x="1"/>
    <square prefix="SS" y="1" x="2"/>
    <square prefix="ST" y="1" x="3"/>
    <square prefix="SU" y="1" x="4"/>
    <square prefix="TQ" y="1" x="5"/>
    <square prefix="TR" y="1" x="6"/>
    <square prefix="SM" y="2" x="1"/>
    <square prefix="SN" y="2" x="2"/>
    <square prefix="SO" y="2" x="3"/>
    <square prefix="SP" y="2" x="4"/>
    <square prefix="TL" y="2" x="5"/>
    <square prefix="TM" y="2" x="6"/>
    <square prefix="SH" y="3" x="2"/>
    <square prefix="SJ" y="3" x="3"/>
    <square prefix="SK" y="3" x="4"/>
    <square prefix="TF" y="3" x="5"/>
    <square prefix="TG" y="3" x="6"/>
    <square prefix="SC" y="4" x="2"/>
    <square prefix="SD" y="4" x="3"/>
    <square prefix="SE" y="4" x="4"/>
    <square prefix="TA" y="4" x="5"/>
    <square prefix="NW" y="5" x="1"/>
    <square prefix="NX" y="5" x="2"/>
    <square prefix="NY" y="5" x="3"/>
    <square prefix="NZ" y="5" x="4"/>
    <square prefix="OV" y="5" x="5"/>
    <square prefix="NR" y="6" x="1"/>
    <square prefix="NS" y="6" x="2"/>
    <square prefix="NT" y="6" x="3"/>
    <square prefix="NU" y="6" x="4"/>
    <square prefix="NL" y="7" x="0"/>
    <square prefix="NM" y="7" x="1"/>
    <square prefix="NN" y="7" x="2"/>
    <square prefix="NO" y="7" x="3"/>
    <square prefix="NF" y="8" x="0"/>
    <square prefix="NG" y="8" x="1"/>
    <square prefix="NH" y="8" x="2"/>
    <square prefix="NJ" y="8" x="3"/>
    <square prefix="NK" y="8" x="4"/>
    <square prefix="NA" y="9" x="0"/>
    <square prefix="NB" y="9" x="1"/>
    <square prefix="NC" y="9" x="2"/>
    <square prefix="ND" y="9" x="3"/>
    <square prefix="HW" y="10" x="2"/>
    <square prefix="HX" y="10" x="3"/>
    <square prefix="HY" y="10" x="4"/>
    <square prefix="HZ" y="10" x="5"/>
    <square prefix="HT" y="11" x="3"/>
    <square prefix="HU" y="11" x="4"/>
    <square prefix="HP" y="12" x="4"/>
</Grid>;
